...
class KeyframeExtractor:
    ...
    def extract(self, video_path: str, document: Document):
        ...
        if not keyframes:
            print(
                "Warning: No keyframes extracted. This may indicate a cv2 error or an unreadable video file."
            )
            return
        ...
        for cluster in range(n_clusters):
            ...
            if cluster_indices.size > 0:
                distances = np.linalg.norm(
                    keyframes_array[cluster_indices] - cluster_centers[cluster], axis=1
                )
                closest_index = cluster_indices[np.argmin(distances)]
                representative_keyframes.append(
                    {
                        "frame": keyframes_array[closest_index].reshape(50, 50, 3),
